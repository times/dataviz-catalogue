...
var svg = d3.select("#times-vertical-line")
            .attr("width", width)
            .attr("height", height)
...
d3.json("data.json", function(err, dataset) {
  ...
  var clubDomain = [d3.min(dataset, function(d) { return d.value; }), d3.max(dataset, function(d) { return d.value; })]

  // Set domains
  // Fixed values x-axis
  x.domain(hardCordedDomain);
  y.domain(d3.extent(dataset, function(d) { return d.year; }).reverse());
  ...
  var swoopy = d3.swoopyDrag()
    .x(function(d){ return x(d.value) })
    .y(function(d){ return y(parseTime(d.year)) })
    //.draggable(true)
